...
wild_matix<-readRDS("../Input_Data/PreprocessingData/wild_matix.R")
cancer_mutations3<-readRDS("../Input_Data/PreprocessingData/cancer_mutations.R")
...
cancers<-c("leukemia","eye","soft_tissue","gastric","cns","kidney","thyroid","skin"                     
  ...
  "prostate","undefined","liver","lymphoma","oesophagus","cervix","biliary_tract")  cancer
l
ibrary(glmmTMB)
for(j in 1:lengthg(cancers)){
cancerd<-cancerd[which(cancerd$cancer==cancers[j] ),]
...
twp2<-twp2[which(twp2[,6]>0),]
twp2<-twp2[order(twp2[,6]),]

write.csv(twp2,paste0("../Input_Data/Output/PancancerResults.csv" , cancers[j],".csv",row.names=FALSE))
}

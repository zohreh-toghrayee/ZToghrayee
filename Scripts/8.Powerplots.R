...
cancelf<-unique(cancer$CLEANNAME)
  ...
  others<-intersect(names(which(wild_matix[which(rownames(wild_matix)=="KRAS"),]==2)),unique(cancer$CLEANNAME))###cellines where the ith gene is not mutated
...
krasd2<-krasd[which(krasd$celline %in% celinesf),]
...
ataris=matrix(0,nrow=length(which(colnames(ATARIS) %in% celinesf)),ncol=4)

colnames(ataris)<-c("score","group","score2","samescale")
ataris[,3]<-0
rownames(ataris)=colnames(ATARIS[which(rownames(ATARIS)=="KRAS"),which(colnames(ATARIS) %in% celinesf)])
ataris[,1]= t(ATARIS[which(rownames(ATARIS)=="KRAS"),which(colnames(ATARIS) %in% celinesf)])
dim(ataris)
...
effects2<-d*sqrt(var(krasd$meanlogfc))
...
###cancer
    ...
    msh2<-summary(glmer(rankv ~mutshufle+(1|CLEANNAME),data=cancer,family="poisson"))
...
KRASpowerplot<-matrix(0,nrow=length(ncm),ncol=4)
KRASpowerplot[,1]<-step
KRASpowerplot[,2]<-powerplotmymethod/100
KRASpowerplot[,3]<-powerataris/100
KRASpowerplot[,4]<-powerraw/100

colnames(KRASpowerplot)<-c("samplesize","Mymethod","ATARIS","powerraw")
KRASpowerplot2<-data.frame(rep(KRASpowerplot[,1],3),c(KRASpowerplot[,2],KRASpowerplot[,3],KRASpowerplot[,4]),rep(1:3,each=6))
